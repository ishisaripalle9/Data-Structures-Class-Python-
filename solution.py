...
class Graph:
    ...
    def bfs(self, begin_id: str, end_id: str) -> Tuple[List[str], float]:
        """
        Perform a breadth-first search beginning at vertex with id begin_id
        ...
        while not q.empty():
            vertex = q.get()
            if vertex == end_id:
                return self.build_path(visited, begin_id, end_id)
            else:
                if self.get_vertex_by_id(vertex):
                    for i in self.get_vertex_by_id(vertex).adj:
                        if not self.get_vertex_by_id(i).visited:
                            self.get_vertex_by_id(i).visited = True
                            visited[i] = vertex
                            q.put(i)
        return [], 0


    def a_star(self, begin_id: str, target_id: str,
               metric: Callable[[Vertex, Vertex], float]) -> Tuple[List[str], float]:
        """
        Perform an A* search beginning at vertex with id
        start_id and terminating at vertex with id end_id
        params: begin_id, target_id, metric
        returns: tuple of the form ([path], distance)
        """
        start = self.get_vertex_by_id(begin_id)
        end = self.get_vertex_by_id(target_id)
        ...
        visited = {}
        distances = {}
        for i in self.get_all_vertices():
            distances[i.id] = float('inf')
            q.push(float('inf'), i)
        q.update(0, start)
        distances[start.id] = 0
        while not q.empty():
            dist, vertex = q.pop()
            if vertex.id == target_id and distances[target_id] != float('inf'):
                return self.build_path(visited, begin_id, target_id)
            else:
                for i in vertex.adj:
                    new_distance = self.get_edge_by_ids(vertex.id, i)[2] + distances[vertex.id]
                    if new_distance < distances[i]:
                        distances[i] = new_distance
                        priority = metric(self.get_vertex_by_id(target_id), self.get_vertex_by_id(i))
                        q.update(priority + new_distance, self.get_vertex_by_id(i))
                        visited[i] = vertex.id
        return [], 0
...
def teleport(system_dict, start, end):
    """
    This function decides the path we will take to reach our destination,
    utilizing any wormholes that allow for instantaneous travel between two
    separate points in space. the function will utilize system_dict
    that will allow us to calculate the shortest path
    params: system_dict, start, end
    returns: distance, a float value representing distance from
    starting vertex to ending vertex
    """
    start_galaxy_name, start_vertex_name = start
    end_galaxy_name, end_vertex_name = end
    giant_graph = Graph()
    if start_galaxy_name not in system_dict or end_galaxy_name not in system_dict:
        return float('inf')
    else:
        for intermediate_galaxy_name, galaxy_info in system_dict.items():
            intermediate_galaxy_graph = galaxy_info['graph']
            intermediate_departure_teleport = galaxy_info['departure_teleport']
            intermediate_arrival_teleport = galaxy_info['arrival_teleport']
            intermediate_departure_destinations = galaxy_info['departure_destinations']
            current_path, within_distance = intermediate_galaxy_graph.a_star(intermediate_arrival_teleport, intermediate_departure_teleport, Vertex.euclidean_distance)
            if current_path is not None:
                for next_system in intermediate_departure_destinations:
                    giant_graph.add_to_graph(intermediate_galaxy_name, next_system, within_distance)
            if intermediate_galaxy_name == start_galaxy_name:
                path, within_distance = intermediate_galaxy_graph.a_star(start_vertex_name, intermediate_departure_teleport, Vertex.euclidean_distance)
                if path:
                    for next_system in intermediate_departure_destinations:
                        giant_graph.add_to_graph(intermediate_galaxy_name + "_start", next_system, within_distance)
                if intermediate_galaxy_name == end_galaxy_name:
                    path, within_distance = intermediate_galaxy_graph.a_star(start_vertex_name, end_vertex_name, Vertex.euclidean_distance)
                    if path:
                        giant_graph.add_to_graph(intermediate_galaxy_name + "_start", end_galaxy_name + "_end", within_distance)
            if intermediate_galaxy_name == end_galaxy_name:
                path, within_distance = intermediate_galaxy_graph.a_star(intermediate_arrival_teleport, end_vertex_name, Vertex.euclidean_distance)
                if path:
                    giant_graph.add_to_graph(intermediate_galaxy_name, end_galaxy_name + "_end", within_distance)
        path, within_distance = giant_graph.a_star(start_galaxy_name + "_start", end_galaxy_name + "_end", Vertex.euclidean_distance)
        if path:
